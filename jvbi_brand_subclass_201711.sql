...
FROM (SELECT tran_date,LOCATION,item,total_retail,total_cost,dept||CLASS||subclass AS subclass FROM tran_data_history WHERE tran_code=1 AND tran_date>=TRUNC(ADD_MONTHS(SYSDATE,-2),'mm')
AND tran_date<=LAST_DAY(ADD_MONTHS(TRUNC(SYSDATE),-2))) A,
jv_store_vw b,
