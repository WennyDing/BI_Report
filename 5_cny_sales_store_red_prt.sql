...
SET TRIMSPOOL ON
set linesize 2000
spool E:\BI_Report\work\cny_sales_store_red_rpt.csv
set verify off
SELECT col_name FROM (
SELECT '店群,门店编号,门店名称,日期,当日未税销售,当日未税销售预算,当日未税销售达成,去年同期未税销售,当日未税销售LFL,CNY累计未税销售,CNY累计未税销售预算,CNY累计未税销售达成,去年CNY累计未税销售,CNY累计未税销售LFL' AS col_name,0 AS order_seq,0 FROM DUAL
UNION
...
SELECT rpt_district_name
     ...
     ||','||sev_lfl_cny_total
     ||','||DECODE(sev_lfl_cny_total,0,0,ROUND(sev_cny_total/sev_lfl_cny_total-1,2))*100||'%' AS col_name,1 AS order_seq
FROM ( 
...
ORDER BY DECODE(sev_budget,0,0,sev/sev_budget) DESC, rpt_district_name, STORE )
ORDER BY 2,3) TT;
spool off;
exit;